...
# Test script for Per-Pedido Fiscal Data UI Changes
...
echo "=============================================="
echo "TEST: Per-Pedido Fiscal Data UI (State Isolation)"
echo "=============================================="
echo
...
create_mesa() {
    ...
    RESPONSE=$(curl -s -X POST "$API_URL/mesas" \
        ...
        -d "{\"codigo\":\"TEST-FISCAL-$RANDOM\",\"capacidad\":4,\"estado\":\"disponible\"}")
...
create_producto() {
    ...
    RESPONSE=$(curl -s -X POST "$API_URL/productos" \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer $TOKEN" \
        -d "{\"nombre\":\"Test Product $RANDOM\",\"descripcion\":\"Test\",\"precio\":15.00,\"categoria\":\"PLATO_PRINCIPAL\",\"disponible\":true}")
...
echo "[STEP 4/4] Generating factura for Pedido #2 (Nominative - Pedro López)..."
RESPONSE_2=$(curl -s -X POST "$API_URL/facturas" \
    ...
    -d "{
        ...
        \"clienteIdentificacionFiscal\":\"9876543210\",
        \"clienteNombre\":\"Pedro López\",
        \"clienteDireccion\":\"Calle de la Prueba 999\",
        \"clienteEmail\":\"pedro@test.com\",
        \"clienteTelefono\":\"555-9999\",
...
if [ "$CLIENTE_NOMBRE_2" = "Pedro López" ] && [ "$CLIENTE_ID_FISCAL_2" = "9876543210" ] && [ "$CLIENTE_DIRECCION_2" = "Calle de la Prueba 999" ]; then
    echo "[SUCCESS] ✓ Factura #2 snapshot is correct (Pedro López nominative data)"
else
...
echo "- Pedido #$PEDIDO_2_ID → Factura #$FACTURA_2_ID: Pedro López (9876543210)"
